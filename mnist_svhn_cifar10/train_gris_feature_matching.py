...
with open("/misc/lmbraid19/mittal/master_thesis/git_repo/master-thesis-work/adversarial_autoencoder/pickles/GRIS_new_splits/train/split32x32/unlabeled/data_15000_1.pkl", 'rb') as pickle_load:
   trainx = np.array(pickle.load(pickle_load))
   trainx = np.swapaxes(trainx, 1, 3)
with open("/misc/lmbraid19/mittal/master_thesis/git_repo/master-thesis-work/adversarial_autoencoder/pickles/GRIS_new_splits/train/split32x32/unlabeled/labels_15000_1.pkl", 'rb') as pickle_load:
   trainy = np.array(pickle.load(pickle_load))
	
with open("/misc/lmbraid19/mittal/master_thesis/git_repo/master-thesis-work/adversarial_autoencoder/pickles/GRIS_new_splits/test/split32x32/selected/data_3000_1.pkl", 'rb') as pickle_load:
   testx = np.array(pickle.load(pickle_load))
   testx = np.swapaxes(testx, 1, 3)
with open("/misc/lmbraid19/mittal/master_thesis/git_repo/master-thesis-work/adversarial_autoencoder/pickles/GRIS_new_splits/test/split32x32/selected/labels_3000_1.pkl", 'rb') as pickle_load:
   testy = np.array(pickle.load(pickle_load))
...
init_param = th.function(inputs=[x_lab], outputs=None, updates=init_updates, allow_input_downcast=True) # data based initialization
train_batch_disc = th.function(inputs=[x_lab,labels,x_unl,lr], outputs=[loss_lab, loss_unl, train_err], updates=disc_param_updates+disc_avg_updates, allow_input_downcast=True)
test_batch = th.function(inputs=[x_lab,labels], outputs=test_err, givens=disc_avg_givens, allow_input_downcast = True)
...
train_batch_gen = th.function(inputs=[x_unl,lr], outputs=None, updates=gen_param_updates, allow_input_downcast=True)
